    ...
    $(function () {
        ...
        $('#btn-{{$modalName}}-delete').click(function () {
            ...
            $.post($(this).data('action'), {
                ...
                showHideLoaderModal();
                var totalPayment = $('.delete-{{$modalName}}-active').closest('tr').closest('tbody').children().length;
                if (totalPayment == 1) {
                    location.reload();
                }
                $('.payment-count').html(totalPayment - 1);
    ...
    <div class="modal-dialog text-break">
        <div class="modal-content">
            <div class="modal-header bg-danger">
                ...
                <button type="button" class="close btn-{{$modalName}}-cancel" data-dismiss="modal" aria-hidden="true">&times;</button>
            </div>
            ...
            <div class="modal-footer pb-1 pt-1">
                <div class="col-sm-12">
                    <button type="button" id="btn-{{$modalName}}-delete"
                            ...
                            data-action="{{ $url }}" data-payment-invoice-id="{{ $paymentInvoiceId }}">
                        {{ trans('fi.delete') }}
                    </button>

                    <button type="button" class="btn btn-sm btn-outline-secondary float-right btn-{{$modalName}}-cancel" data-dismiss="modal"
