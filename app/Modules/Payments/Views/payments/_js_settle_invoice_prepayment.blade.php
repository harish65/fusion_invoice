...
<script type="text/javascript">
    $(function () {
        ...
        function submitFetchInvoicesForm() {
            $.post('{{ route('payments.storeInvoiceSettlementWithPrePayment') }}', $('#fetch-invoices-form').serialize(), {}).done(function (response) {
        ...
        $('#confirm-payment-invoices').on("click", function () {
            ...
            if ($("input:checkbox:checked[id^='prepayment_selection_']").length > 0) {
                if (parseFloat(remaining_balance)) {
                    ...
                    $('#modal-confirm-payment-invoices').load('{!! route('confirm.creditMemo.and.prePayments.invoice.modal') !!}', {
                            action: '{{ route('payments.storeInvoiceSettlementWithPrePayment') }}',
                            header: header,
                            confirmRemainingBalance: confirm_remaining_balance,
                            modalName: 'pre-payments-invoice',
                            flag: true,
                            color: color,
                            returnURL: false
                        },
        ...
        const selectInvoice = (invoice_id) => {
            ...
            return new Promise((resolve, reject) => {
                ...
                let currency = $('#prepayment_selection_' + invoice_id).data('currency');
                let fetch_full_amount = parseFloat($('#prepayment_selection_' + invoice_id).data('amount'));

                let remaining_balance = parseFloat($('#remaining_balance').data('amount'));

                if ($('#prepayment_selection_' + invoice_id).prop("checked")) {
        ...
        $("input:checkbox[id^='prepayment_selection_']").click(function () {
        ...
        function countRemainingBalance() {
            ...
            $("input:checkbox:checked[id^='prepayment_selection_']").each(function () {
