...
<script type="text/javascript">
    $(function () {
        ...
        function submitFetchInvoicesForm() {
            $.post('{{ route('payments.storeInvoiceSettlementWithCreditMemo') }}', $('#fetch-invoices-form').serialize(), {})
        ...
        $('#confirm-payment-invoices').on("click", function () {
            ...
            if (remaining_balance <= 0) {
                var confirm_remaining_balance = "{!! trans('fi.confirm_full_credit_memo_applied') !!}";
                var header = '{!! trans('fi.remaining_balance') !!}';
                var color = 'success';

            } else {
                var confirm_remaining_balance = "{!! trans('fi.confirm_remaining_balance_invoice') !!}";
                confirm_remaining_balance = confirm_remaining_balance.replace(':value', "{{getCurrencySign($invoice->currency_code)}}" + ' ' + systemCurrencyFormat(remaining_balance));
                var header = '{!! trans('fi.remaining_balance') !!}';
                var color = 'danger';
            }

            if ($("input:checkbox:checked[id^='creditmemo_selection_']").length > 0) {
                if (parseFloat(remaining_balance)) {
                    $('#modal-confirm-payment-invoices').load('{!! route('confirm.creditMemo.and.prePayments.invoice.modal') !!}', {
                            action: '{{ route('payments.storeInvoiceSettlementWithCreditMemo') }}',
                            header: header,
                            confirmRemainingBalance: confirm_remaining_balance,
                            modalName: 'credit-memo-invoice',
                            flag: true,
                            color: color,
                            returnURL: false
                        },
                ...
                $('#modal-confirm-payment-invoices').load('{!! route('confirm.creditMemo.and.prePayments.invoice.modal') !!}', {
                        ...
                        confirmRemainingBalance: "{!! trans('fi.invoice_not_selected_warning') !!}",
                        modalName: 'credit-memo-invoice',
                        flag: false,
                        color: 'danger',
                        returnURL: false
        ...
        const selectInvoice = (invoice_id) => {
            ...
            return new Promise((resolve, reject) => {
                let currency = $('#creditmemo_selection_' + invoice_id).data('currency');
                let fetch_full_amount = parseFloat($('#creditmemo_selection_' + invoice_id).data('amount'));

                let remaining_balance = parseFloat($('#remaining_balance').data('amount'));

                if ($('#creditmemo_selection_' + invoice_id).prop("checked")) {

                    $('#confirm-payment-invoices').prop('disabled', false)
                    ...
                    if ($('.credit-memo-checkbox:checked').length == 0) {
                        $('#confirm-payment-invoices').prop('disabled', true)
                    }
        ...
        $("input:checkbox[id^='creditmemo_selection_']").click(function () {
        ...
        $("input[id^='paid_amount_']").blur(function () {
            ...
            if (remaining_balance < 0) {
                $(this).val(systemCurrencyFormat(0.00));
                alertify.alert().setHeader(importantNoteHeader).set({transition: 'zoom'})
                    .setContent("{!! trans('fi.more_figure_then_invoice_total') !!}").showModal();
        ...
        function countRemainingBalance() {
            ...
            $("input:checkbox:checked[id^='creditmemo_selection_']").each(function () {
