...
@section('javascript')
    ...
    <script type="text/javascript">
        $(function () {
            ...
            initDateRangePicker('tax_summary');

            initDateRangePreSelected('tax_summary');
            ...
            $('#btn-run-report').click(function () {
                ...
                var from_date = $('#tax_summary_from_date').val();
                var to_date = $('#tax_summary_to_date').val();
                ...
                $.post("{{ route('reports.taxSummary.validate') }}", {
                    ...
                    query_string = "?from_date=" + from_date + "&to_date=" + to_date + "&company_profile_id=" + company_profile_id + "&exclude_unpaid_invoices=" + exclude_unpaid_invoices + "&date_filter_by=" + date_filter_by;
                    if (output_type == 'preview') {
                        $('#report-preview').show();
                        $('#preview-results').attr('src', "{{ route('reports.taxSummary.html') }}" + query_string);
                    }
                    else if (output_type == 'pdf') {
                        window.open("{{ route('reports.taxSummary.pdf') }}" + query_string, '_blank');
                    }
                    else if (output_type == 'csv') {
                        window.open("{{ route('reports.taxSummary.csv') }}" + query_string, '_blank');
                    }
...
@section('content')
    ...
    <section class="content-header">
        <div class="container-fluid">
            <div class="row">
                <div class="col-6">
                    <h1><i class="fa fa-bar-chart"> </i> {{ trans('fi.tax_summary') }}</h1>
                </div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-2">
                                    <div class="form-group">
                                        ...
                                        {!! Form::hidden('from_date', null, ['id' => 'tax_summary_from_date']) !!}
                                        {!! Form::hidden('to_date', null, ['id' => 'tax_summary_to_date']) !!}
                                        {!! Form::text('date_range', null, ['id' => 'tax_summary_date_range', 'class' => 'form-control form-control-sm', 'readonly' => 'readonly','placeholder'=>trans('fi.filter_by_date')]) !!}
                                    </div>
                                </div>
                                <div class="col-md-2">
                                    <div class="form-group">
