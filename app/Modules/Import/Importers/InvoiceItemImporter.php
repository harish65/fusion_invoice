...
class InvoiceItemImporter extends AbstractImporter
{
    public function getFields()
    {
        ...
        foreach (CustomField::forTable('invoice_items')->get() as $customField)
        {
    ...
    {
        $this->file = storage_path('invoiceItems.csv');
        $row        = 1;
            ...
            DB::transaction(function () use (&$handle, &$fields, &$response, &$taxRates, &$customFields, &$row, &$labels)
            {
                while (($data = fgetcsv($handle, $this->max_records, ',')) !== false)
                {
                    if ($row !== 1 && $data !== array(null))
                    {
                        ...
                        $invoice = Invoice::where('number', $record['invoice_number'])->first();
                        ...
                        if ($invoice)
                        {
                            ...
                            if ($this->validateRecord($record, $row))
                            {
                                ...
                                $invoiceItem = $invoice->items()->create($record);
                                ...
                                if ($customFields)
                                {
                                    ...
                                    $invoiceItem->custom->update($customRecord);
